                            ...
                            <div class="am-tutorsearch_video">
                                @if(!empty($tutor->profile->intro_video))
                                    <video class="video-js" data-setup='{}' preload="auto" wire:key="profile-video-{{ $tutor->id }}" id="profile-video-{{ $tutor->id }}" width="320" height="240" controls >
                                        <source src="{{ asset('storage/'.$tutor->profile->intro_video).'?key=profile-video'. $tutor->id }}#t=0.1" wire:key="profile-video-src-{{ $tutor->id }}" type="video/mp4" >
                                    </video>
                                @elseif(!empty($tutor->profile->image) && Storage::disk('public')->exists($tutor->profile->image))
                                    <img src="{{ asset('storage/'.$tutor->profile->image) }}" alt="{{$tutor->profile->full_name}}" />
                                @else
                                    <img src="{{ asset('images/placeholders/placeholder-land.png') }}" alt="{{ $tutor->profile->full_name }}" />
                                @endif
                                ...
                                <div class="am-tutorsearch_head">
                                    <div class="am-tutorsearch_user">
                                        <figure class="am-tutorvone_img">
                                            ...
                                            @else
                                                <img src="{{ resizedImage('placeholder.png',50,50) }}" alt="{{ $tutor->profile->full_name }}" />
                                            @endif
