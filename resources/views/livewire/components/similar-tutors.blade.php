                        ...
                        <div class="am-similar-user">
                            <div class="am-tutordetail_user">
                                <figure class="am-tutorvone_img">
                                    @if (!empty($item->profile?->image) && Storage::disk('public')->exists($item->profile?->image))
                                    <img src="{{ resizedImage($item->profile?->image,40,40) }}" alt="{{$item->profile?->image}}" />
                                    @else
                                        <img src="{{ resizedImage('placeholder.png',40,40) }}" alt="{{ $item->profile?->image }}" />
                                    @endif
                                </figure>
                            ...
                            <ul class="am-tutorreviews-list">
                                <li>
                                    <div class="am-tutorreview-item">
                                        @php
                                            $formattedValue = number_format($item->min_price, 2);
                                            list($integerPart, $decimalPart) = explode('.', $formattedValue);
                                        @endphp
                                            <div class="am-tutorreview-item_icon"><i class="am-icon-dollar"></i></div>
                                            <span class="am-uniqespace">{{ formatAmount($integerPart) }} <em>{{ __('tutor.session_text') }}</em></span>
                                    </div>
                                </li>
                                <li>
                                    <div class="am-tutorreview-item">
