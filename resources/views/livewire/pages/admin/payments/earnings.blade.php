<main class="tb-main">
                ...
                <table class="table tb-table tb-dbholder @if(setting('_general.table_responsive') == 'yes') tb-table-responsive @endif">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td data-label="{{ __('transaction.ref_no' )}}"><span>{{
                                    !empty($single->transaction->trans_ref_no) ? $single->transaction->trans_ref_no :
                                    $single->transaction->invoice_ref_no }}</span></td>
                            <td data-label="{{ __('transaction.created_date' )}}"><span>{{ date($date_format, strtotime(
                                    $single->transaction->updated_at )) }}</span></td>
                            <td data-label="{{ __('transaction.total_amount' )}}">
                                <span>{{getPriceFormat($currency_symbol, $single->transaction->TransactionDetail->amount
                                    + $single->transaction->TransactionDetail->used_wallet_amt) }}</span>
                            </td>
                            <td data-label="{{ __('transaction.admin_fees' )}}"><span>{{getPriceFormat($currency_symbol,
                                    $single->amount) }}</span></td>
                            <td data-label="{{ __('transaction.payment_type' )}}"><span>{{
                                    ucfirst($single->transaction->payment_type) }}</span></td>
                            <td data-label="{{ __('transaction.payment_method' )}}"><span>{{
                                    ucfirst($single->transaction->payment_method) }}</span></td>
                            ...
                            <td data-label="{{__('general.actions')}}">
                                <ul class="tb-action-status">
                                    <li>
                                        <a href="javascript:;"
                                            wire:click.prevent="earningDetail({{ $single->transaction->id }})"><i
                                                class="icon-eye"></i></a>
                                    </li>
                                </ul>
                            </td>
    ...
    <div wire:ignore.self class="modal fade tb-addonpopup" id="transaction_detail_popup" tabindex="-1" role="dialog"
        aria-hidden="true">
                ...
                <div class="modal-body">
                    ...
                    <ul class="tb-userinfo-list" id="{{$modal_id}}">
                        ...
                        <li>
                            ...
                            <h6>{{getPriceFormat($transaction_detail['currency'], $transaction_detail['total_amount'])
                                }}</h6>
                        </li>
                        @if( $transaction_detail['type'] == 'project' && !empty($transaction_detail['seller_amount']) )
                        <li>
                            <span>{{ __('transaction.seller_amount') }}:</span>
                            <h6>{{getPriceFormat($transaction_detail['currency'], $transaction_detail['seller_amount'])
                                }}</h6>
                        </li>
                        <li>
                            <span>{{ __('transaction.admin_amount') }}:</span>
                            <h6>{{getPriceFormat($transaction_detail['currency'], $transaction_detail['admin_amount'])
                                }}</h6>
                        </li>
                        @endif
                        <li>
