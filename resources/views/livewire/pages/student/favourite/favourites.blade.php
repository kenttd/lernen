            ...
            <div class="am-resume">
                @foreach($favourites as $favourite)
                    <div class="am-resume_item am-resume_wrap">
                            @if (!empty($favourite->profile->image) && Storage::disk('public')->exists($favourite->profile->image))
                            <img src="{{ resizedImage($favourite->profile->image,50,50) }}" alt="{{$favourite->profile->image}}" />
                            @else
                                <img src="{{ resizedImage('placeholder.png',50,50) }}" alt="{{ $favourite->profile->image }}" />
                            @endif
