                    ...
                    <tbody>
                        @foreach($certificates as $key => $certificate)       
                            <tr>
                                ...
                                <td data-label="{{ __('general.title') }}"><span>{{$certificate?->template->title }}</span></td>
                                <td data-label="{{ __('calendar.subject') }}"><span>{{$certificate?->wildcard_data['subject_name'] }}</span></td>
                                <td data-label="{{ __('general.date') }}"><span>{{ $certificate?->created_at->format(setting('_general.date_format')) }}</span></td>
                                <td data-label="{{ __('general.issued_by' )}}">
                                    <span>{{$certificate?->wildcard_data['issued_by'] ?? 'N/A' }}</span>
                                </td>
                                <td data-label="{{ __('general.actions' )}}">
                                    <div class="am-invoicetable_actions">
                                        <a href="{{ route('upcertify.certificate', $certificate?->hash_id) }}" target="_blank" class="am-custom-tooltip">
                                            <span class="am-tooltip-text"><span>View</span></span><i class="am-icon-eye-open-01"></i>
                                        </a>
                                        <a href="{{ route('upcertify.download', $certificate?->hash_id) }}" class="am-custom-tooltip">
                                            <i class="am-icon-download-01"></i>
                                            <span class="am-tooltip-text"><span>download</span></span>
                                        </a>
                                    </div>
                                </td>
                            </tr>
