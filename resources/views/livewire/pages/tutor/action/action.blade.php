...
new #[Layout('layouts.app')] class extends Component
{
    ...
    public function mount($tutor,$isFavourite,$navigate = true)
    {
        ...
        if (!empty($tutor->profile->image) && Storage::disk('public')->exists($tutor->profile->image)) {
...
<div x-data="{
    ...
    @else
    <a href="javascript:void(0);" onclick="Livewire.dispatch('showAlertMessage', {type: `error`, message: `{{ Auth::check() ?  __('general.not_allowed') : __('general.login_error') }}` })" class="am-white-btn"> {{ __('tutor.send_message') }} <i class="am-icon-chat-03"></i></a>
    <a href="javascript:void(0);" onclick="Livewire.dispatch('showAlertMessage', {type: `error`, message: `{{ Auth::check() ?  __('general.not_allowed') : __('general.login_error') }}` })" class="am-likebtn"> <i class="am-icon-heart-01"></i> </a>
    @endrole
