        ...
        <form wire:submit="updateInfo" class="am-themeform am-themeform_personalinfo">
            ...
            @else
                <fieldset>
                    ...
                    <div class="form-group @error('form.description') am-invalid @enderror">
                        ...
                        <div class="am-editor-wrapper">
                            <div class="am-custom-editor am-custom-textarea" wire:ignore>
                                <textarea id="profile_desc" class="form-control" placeholder="{{ __('profile.description_placeholder') }}">{{ $form->description }}</textarea>
                        ...
                        <div class="am-uploadoption" x-data="{isUploading:false}" wire:key="uploading-img-{{ time() }}">
                            ...
                            @if(!empty($form->image))
                                <div class="am-uploadedfile">
                                    <img src="{{ $form->isBase64 ? $form->image :  asset('storage/'. $form->image ) }}">
                                    @if( $form->isBase64 )
                                        <span>{{ $form->imageName }}</span>
                                    @else
                                        <span>{{ basename(parse_url(asset('storage/'.$form->image), PHP_URL_PATH)) }}</span>
                                    @endif
                    ...
                    @role('tutor')
                        <div class="form-group">
                            <x-input-label class="am-important" for="covervideo" :value="__('profile.intro_video')" />
                            <div class="am-uploadoption profile-img-alpine" x-data="{isUploading:false}" wire:key="uploading-video-{{ time() }}">
                                ...
                                @if(!empty($form->intro_video) || !empty($introVideo))
                                    @php
                                        $videoUrl = !$errors->has('introVideo') && !empty($introVideo) && method_exists($introVideo,'temporaryUrl') ? $introVideo->temporaryUrl() : asset('storage/'.$form->intro_video);
                                    @endphp
                                    @if(!empty($videoUrl))
                                        <div class="am-uploadedfile">
                                            @if(!empty($form->intro_video))
                                                <a href="{{ asset('storage/'.$form->intro_video) }}" data-vbtype="iframe" data-gall="gall" class="tu-themegallery tu-thumbnails_content">
                                                    <figure>
