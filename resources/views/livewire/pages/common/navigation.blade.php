        ...
        <div class="am-wallet">
            <div class="am-wallet_title">
                ...
                <div class="am-wallet_balance">
                    <strong><sup>{!! getCurrencySymbol() !!}</sup>{{ number_format($balance,2)}}<span>{{ __('general.wallet_balance') }}</span></strong>
                </div>
            </div>
            <a href="javascript:void(0);" wire:click="openModel"  class="am-wallet_withdraw {{ empty($userPayoutMethod) || empty($balance) || auth()->user()->hasRole('student') ? 'am-btn_disable' : '' }}">
