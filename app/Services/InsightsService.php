...
class InsightsService
{
    ...
    public function getPlatformEarnings($revenueStartDate = null, $revenueEndDate = null)
    {
        if (!empty($revenueStartDate) && !empty($revenueEndDate)) {
            return OrderItem::whereBetween('created_at', [$revenueStartDate . ' 00:00:00', $revenueEndDate . ' 23:59:59'])->sum('total');
        }

        return OrderItem::sum('total');
    }
    ...
    {
        if (!empty($revenueStartDate) && !empty($revenueEndDate)) {
            return OrderItem::whereBetween('created_at', [$revenueStartDate . ' 00:00:00', $revenueEndDate . ' 23:59:59'])->sum('platform_fee');
        }
