...
class TutorSessions extends Component
{
    ...
    public function bookSession($id)
    {
        ...
        if(!empty($slot)){
            if( $slot->total_booked < $slot->spaces) {
                ...
                Cart::add($data['id'], $data['subject'] , 1, $slot->session_fee, $data);
                $this->dispatch('cart-updated', cart_data: Cart::content(), total: Cart::total(), subTotal: Cart::subtotal());
    ...
    {
        $this->bookingService->removeReservedBooking($params['id']);
    ...
    {
        Cart::clear();
        $this->dispatch('cart-updated', cart_data: Cart::content(), total: Cart::total(), subTotal: Cart::subtotal(), toggle_cart: 'close');
        if(Auth::check()){
