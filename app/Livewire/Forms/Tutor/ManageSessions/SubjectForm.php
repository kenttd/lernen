...
class SubjectForm extends Form
{
    ...
    public function addNewSubject($data)
    {
        ...
        if( !empty($this->image) && method_exists($this->image,'temporaryUrl')){
            $fileName = uniqueFileName('public/subjects', $this->image->getClientOriginalName());
            $url = $this->image->storeAs(path: 'public/subjects', name: $fileName);
            $image = str_replace('public/', '', $url);
            $subject['image'] = $image;
        }
